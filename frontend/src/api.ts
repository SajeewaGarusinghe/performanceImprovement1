...
export const api = {
  ...
  lookupBefore: (size: number, target: number, repeats: number) => axios.get<PerfResponse>(`${API_BASE}/lookup/before`, { params: { size, target, repeats } }).then(r => r.data),
  lookupAfter: (size: number, target: number, repeats: number) => axios.get<PerfResponse>(`${API_BASE}/lookup/after`, { params: { size, target, repeats } }).then(r => r.data),
